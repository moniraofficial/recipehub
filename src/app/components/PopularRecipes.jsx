...
export default function PopularRecipes() {
  ...
  useEffect(() => {
    fetch("https://recipehub-sigma-three.vercel.app /api/recipes/popular")
      .then(res => res.json())
      .then(data => {
        setRecipes(data);
        setLoading(false);
  ...
  if (loading) return <div className="text-center py-6 text-xs text-gray-400">Loading popular...</div>;
  ...
  return (
    <section className="max-w-7xl mx-auto px-6 w-full">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-lg font-black text-gray-950 dark:text-white">Popular Recipes</h2>
          <p className="text-xs text-gray-400">Most liked recipes by our community</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {recipes.map(recipe => (
          <div key={recipe._id} className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-sm hover:shadow-md transition-all p-3 flex flex-col justify-between">
            <div>
              <div className="relative rounded-2xl overflow-hidden">
                <img src={recipe.imageURL} alt={recipe.recipeName} className="w-full h-40 object-cover" />
                <span className="absolute top-3 left-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white font-extrabold px-2 py-0.5 rounded-md text-[8px] uppercase tracking-wider">
                  Popular
                </span>
              </div>
