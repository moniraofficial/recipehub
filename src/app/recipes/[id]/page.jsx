...
import React, { useEffect, useState, use } from 'react';
import { useRouter } from 'next/navigation';
import { authClient } from '../../lib/auth-client'; // Integrated Auth Check
...
export default function RecipeDetailsPage({ params: paramsPromise }) {
  ...
  useEffect(() => {
    ...
    const fetchRecipeDetailsAndStatus = async () => {
      try {
        // 1. Fetch main Recipe Schema details
        ...
        // 2. If logged in, check if user has already liked/favorited/reported this recipe
        if (user?.email) {
          const normalizedEmail = user.email.toLowerCase().trim();
          const userStatusRes = await fetch(`${API_BASE_URL}/api/recipes/${id}/user-status?email=${normalizedEmail}`);
          if (userStatusRes.ok) {
            const status = await userStatusRes.json();
            setIsLiked(status.isLiked || false);
            setIsFavorite(status.isFavorite || false);
            setIsReported(status.isReported || false);
          }
        }
      } catch (err) {
    ...
    fetchRecipeDetailsAndStatus();
  }, [id, user?.email, API_BASE_URL]);

  // Handle Like Toggle Database Operations
  const handleLikeToggle = async () => {
    if (!user) return toast.error("Please log in to like recipes.");

    try {
      const email = user.email.toLowerCase().trim();
      const endpoint = isLiked ? '/api/likes/remove' : '/api/likes/add';
      
      const res = await fetch(`${API_BASE_URL}${endpoint}`, {
        method: isLiked ? 'DELETE' : 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, recipeId: id })
      });
  ...
  // Fixed Favorite Toggle to match your backend REST pattern
  const handleFavoriteToggle = async () => {
    ...
    try {
      ...
      if (res.ok) {
        setIsFavorite(!isFavorite);
        toast.success(isFavorite ? "Removed from favorites" : "Added to favorites dashboard!");
      } else {
        // Fallback fallback matching query string syntax on direct DELETE operations
        if (isFavorite && res.status === 404) {
          const fallbackRes = await fetch(`${url}?email=${email}&recipeId=${id}`, { method: 'DELETE' });
          if (fallbackRes.ok) {
            setIsFavorite(false);
            return toast.success("Removed from favorites");
          }
        }
        toast.error("Failed to save favorite record.");
      }
    } catch (err) {
      console.error("Favorite operational error:", err);
      toast.error("Server synchronization error.");
    }
  };

  // Handle Report Submission with persistent saving
  const handleReportSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/reports/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      console.error("Reporting network failure:", err);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 dark:bg-slate-950 py-6 transition-colors duration-300 relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 space-y-6">
        ...
        {/* Breadcrumbs */}
        <div className="text-xs text-slate-400 font-medium space-y-1">
          <div>Home / Browse Recipes / <span className="font-semibold text-emerald-600">{recipe.recipeName || recipe.name}</span></div>
        </div>
          ...
          <div className="md:col-span-7 space-y-5">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs text-slate-500 dark:text-slate-400 font-medium">
                ...
                <span className="flex items-center gap-1">⏱️ {recipe.preparationTime || recipe.prepTime || 30} min</span>
                <span className="flex items-center gap-1">💪 {recipe.difficultyLevel || recipe.difficulty || 'Easy'}</span>
                <span className="flex items-center gap-1 text-amber-500 font-semibold">⭐ Likes ({likesCount})</span>
              </div>
            </div>

            <p className="text-xs md:text-sm text-slate-500 dark:text-slate-400 leading-relaxed max-w-xl">
              {recipe.description || "Classic delicious meal prepared with fresh ingredients, signature spices, and a fine touch of culinary expertise."}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              ...
              {/* Favorite Button (Now fully operational with local backend) */}
              <button 
                onClick={handleFavoriteToggle}
                className={`flex items-center gap-1.5 px-4 py-2 border font-bold rounded-xl text-xs transition-all ${
                  isFavorite 
        ...
        {/* Bottom Split Layout blocks */}
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
          {/* Ingredients */}
          <div className="md:col-span-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 className="font-bold text-sm text-slate-900 dark:text-white">Ingredients</h3>
            <ul className="space-y-3 text-xs text-slate-600 dark:text-slate-400 font-medium">
              {recipe.ingredients && Array.isArray(recipe.ingredients) ? (
                recipe.ingredients.map((ing, i) => (
          ...
          {/* Instructions */}
          <div className="md:col-span-8 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-5 shadow-sm space-y-4">
            <h3 className="font-bold text-sm text-slate-900 dark:text-white">Instructions</h3>
            <ol className="space-y-4 text-xs text-slate-600 dark:text-slate-400 font-medium">
              {recipe.instructions ? (
                typeof recipe.instructions === 'string' ? (
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl max-w-md w-full border border-slate-100 dark:border-slate-800 shadow-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
              <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-1.5">🏴 Report Recipe</h3>
            ...
            <form onSubmit={handleReportSubmit} className="p-6 space-y-4">
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                ...
                <button type="submit" className="bg-emerald-600 text-white font-bold px-4 py-2 rounded-xl text-xs">Submit Report</button>
              </div>
            </form>
          </div>
        </div>
